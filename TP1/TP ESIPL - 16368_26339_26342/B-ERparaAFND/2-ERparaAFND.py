...
# Args : expressao_regular (dict) : expressão regular como um dicionário Python
...
def converter_afnd(expressao_regular):
    ...
    # Função para realizar a operação de transição transitiva num AFND, para usar o trans "+" na expressão regular
    # Args : afnd (dict) : O AFND como um dicionário Python
    # Returns : dict : O AFND resultante da transição transitiva
    def transitiva(afnd):
        novo_estado_inicial = criar_estado()
        novo_estado_final = criar_estado()
        
        novo_delta = {}
        for estado in afnd["Q"]:
            transicoes = afnd["delta"].get(estado, {})
            transicoes.setdefault("", []).append(novo_estado_final)
            novo_delta[estado] = transicoes
    ...
    def converter_estadoER(estadoER):
        ...
        else:  # Se o nó representa um operador (seq, alt, kle ou trans) então...
